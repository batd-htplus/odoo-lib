...
class HtplusAiForecast(models.Model):
    _name = 'htplus.ai.forecast'
    ...
    line_ids = fields.One2many('htplus.ai.forecast.line', 'forecast_id', string='Lines')
    ai_job_id = fields.Char(string='AI Job ID')
    company_id = fields.Many2one('res.company', default=lambda self: self.env.company)

    @api.model
    def poll_pending_jobs(self):
        pending = self.search([('state', '=', 'draft'), ('ai_job_id', '!=', False)])
        for forecast in pending:
            result = self.env['htplus.ai.service'].poll_job(forecast.ai_job_id)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('htplus.ai.forecast') or _('New')
        return super().create(vals_list)

    def action_run(self):
        self.ensure_one()
        if not self.config_id:
            self.config_id = self.env['htplus.ai.config']._get_active()
        history = [{
            'date': line.date.isoformat(),
            'product_id': line.product_id.id,
            'qty': line.qty,
        } for line in self.env['htplus.demand.plan.line'].search([], limit=1000)]
        result = self.env['htplus.ai.service'].forecast(
            self.product_ids.ids, self.horizon_days, history)
        if result.get('forecast_id'):
            self.ai_job_id = result['forecast_id']
            return True
        lines = [(0, 0, {
            'product_id': item['product_id'],
            'date': item['date'],
...
class HtplusAiForecastLine(models.Model):
    _name = 'htplus.ai.forecast.line'
    _description = 'AI Forecast Line'

    forecast_id = fields.Many2one('htplus.ai.forecast', required=True, ondelete='cascade')
